...
public class NameListView extends BorderPane {
	...
	private Button addNumberButton, removeNumberButton, removeButton;
	...
	private void remove(){
		...
		if(index != -1) {
			String name = obsList.get(index);
			if(Dialogs.confirmDialog("","Hello", "Are you sure you wanna delete " + name)) {
	...
	private void removeNumber() {
		...
		if (index != -1) {
			String name = obsList.get(index);
			Optional<String> result = Dialogs.oneInputDialog("Remove number from " + name,"Enter number to remove " , "Number");
			if(result.isPresent()) {
				phoneBook.removeNumber(name,result.get().toString());
				select(index);
			}
		}
	}
